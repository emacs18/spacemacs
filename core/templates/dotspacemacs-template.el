...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   ;; Whether to enable purpose-mode or not. This mode over-rides emacs window
   ;; display in an intrusive way so that it prevents built-in variables such as
   ;; display-buffer-alist from working. Set this to nil to prevent such
   ;; complications. *org-roam* buffer may not display correctly unless this is
   ;; set to non-nil.
   dotspacemacs-activate-window-purpose-mode nil
  ...
  (setq-default
   ...
   `(
     ...
     (c-c++
      ...
      ;; Don't enable cmake to avoid annoying error messages. -rk 1/11/2018
      ;; c-c++-enable-cmake-ide-support t
      ;; c-c++-enable-google-newline nil
      ;; c-c++-enable-google-style nil
      ;; c-c++-enable-rtags-completion t

      ;; Cache may use several GB of disk space!
      c-c++-lsp-cquery-cache-directory ,(expand-file-name "~/.cache/lsp-ccls")
      ;; c-c++-lsp-cquery-cache-directory ,(expand-file-name "~/.cache/lsp-clangd")
      ;; lsp-clients-clangd-executable "clangd-15"
      ;; lsp-clients-clangd-args
      ;; '("--log=verbose"
      ;;   "--pretty"
      ;;   ;; "--query-driver=/usr/bin/clang-xxx,/opt/omneon/cross_x86_64_v10f/bin/g++"
      ;;   )
      ...
      ;; c-c++-lsp-executable "/u/kimr/depot/ccls-20190520/bin/ccls"
      ;; c-c++-lsp-sem-highlight-method 'font-lock ; or 'overlay
      c-c++-lsp-sem-highlight-rainbow t

      ;; ccls-executable ,(expand-file-name "ccls" invocation-directory)
      ;; ccls-args '("--init={\"index\": {\"threads\": 2}}")
      )
     command-log
     ;; csv
     dap
     (dash :variables dash-docs-docsets-path "/u/kimr/public_html/docsets")
     eglot
     (elfeed
      ;; :variables rmh-elfeed-org-files ,(list "~/.emacs.d/data/elfeed-org.org")
      )
     (emacs-lisp :variables emacs-lisp-format-on-save nil)

     (ess :variables ess-r-backend 'lsp)

     ;; I don't like that evil-sniptes takes over 's' key binding in dired-mode
     ;; which is one of my most often used keys. -rk 9/7/2020
     ...
     (javascript :variables
                 javascript-repl `nodejs  ;; nodejs, skewer
                 javascript-backend 'lsp  ;; lsp or tern
                 javascript-fmt-on-save t
                 javascript-fmt-tool 'web-beautify  ;; prettier or web-beautify
     ...
     ;; ;; Disable file watchers to prevent potential slow downs.
     ;; (lsp :variables
     ...
     (org
      ...
      ;; Enabled notification to test drive. -rk 10/16/2021
      ;; org-enable-notifications t
      ;; org-start-notification-daemon-on-startup t

      org-enable-roam-support t       ;; added -rk 10/17/2020
      ...
      org-enable-valign nil           ;; added -rk 12/25/2020
      ;; org-enable-org-journal-support t ; TODO
     ...
     ;; Prevent LSP for bash which does not work!
     ...
     (treemacs :variables
               ;; I'm not so sure what "scope" means.
     ...
     vterm
     web-beautify  ;; for json
     yaml
     ;; my layers
     ;; org-roam  ; disabled in favor of org-enable-roam-support
     )

   dotspacemacs-additional-packages
   '(package-build hook-helpers pretty-hydra
                   helm-org-ql emacsql)

   dotspacemacs-install-packages 'used-but-keep-unused
   ))

;; Use advice rather than making edits above in order to minimize chances of
;; conflicts with upstream changes.
(define-advice dotspacemacs/init (:after (&rest _) my-customization)
  (setq-default
   dotspacemacs-activate-smartparens-mode nil
   dotspacemacs-enable-load-hints t
   dotspacemacs-enable-server t
   dotspacemacs-scratch-mode 'lisp-interaction-mode
   dotspacemacs-themes '(modus-operandi spacemacs-light spacemacs-dark)
   dotspacemacs-which-key-position 'right
   dotspacemacs-install-packages 'used-but-keep-unused
   ))

;; Saving and loading environment is too risky when many different machines use
;; the same emacs files! Perhaps the environment file name should include the
;; hostname.
(define-advice dotspacemacs/user-env (:around (f &rest _) disable-this))
